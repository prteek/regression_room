...
abstract: |
  This post explains a simple, practical way to forecast how many people will use an app each day, week, and month. We describe users with a handful of common-sense states (new, active, at risk, dormant) and model how they move between these states over time. Using a transition-based approach (a Markov chain) together with a modest model for estimating future new users, we simulate the app’s activity and explore how changes like reminders or reward campaigns might affect results. The goal is to give product teams a transparent, data-backed framework for planning growth and retention, without heavy math or jargon.
---
