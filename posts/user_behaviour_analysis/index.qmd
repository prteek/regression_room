...
description: "Analysing and modelling state changes in customer journey to better predict DAU"
...
We use a simulated dataset based on historical data of a SaaS app. <br/> The data is available [here](https://gitlab.com/prteek/regression_room/-/raw/main/posts/user_behaviour_analysis/dau_data.csv) and contains three columns: **user_id, date, and registration_date**.
...
lmod <- glm.nb(
  new_user_count ~ year * bs(week_of_year, 4) + month + day_of_week,
  data = new_users
)
...
new_users_predicted <- tibble(
  date = seq(prediction_start_date, prediction_end_date, by = 'day')
) %>%
  mutate(
    year = year(date),
    week_of_year = isoweek(date), # ISO 8601 week (Monday starts the week)
...
p2 <- ggplot(new_users %>% filter(date >= as.Date('2023-07-01'))) +
  ...
  geom_line(
    data = new_users_predicted %>% filter(date < as.Date('2024-01-01')),
    aes(x = date, y = predicted),
    colour = 'red'
  ) +
  geom_ribbon(
...
The volume of data at our disposal makes this a challenging task to run conveniently on a small machine, so the state determination is performed offline and the code to do it can be found [here](https://gitlab.com/prteek/regression_room/blob/main/posts/user_behaviour_analysis/prepare_states.r)
...
dau_predicted <- predict_dau(
  ...
  prediction_start_date,
  prediction_end_date,
  new_users_predicted %>% mutate(predicted = predicted * 0.8)
)
