...
The diagnostics look just about okay at this point, there is slightly higher dispersion than expected as indicated by the dispersion parameter, `{r} (residuals(glmod_base, type='pearson')^2/df.residual(glmod_base)) |> sum() |> round(2)`
...
df_plot <- df_coef %>%
  ...
  cross_join(tibble(
    percentage_increase_in_population = seq(-1, 1, by = 0.1)
  )) %>%
  mutate(
    age_band = case_when(
    ...
    change_in_odds = (exp(coefficient * percentage_increase_in_population) -
      1) *
      100
  )
...
ggplot(
  ...
  geom_line(
    aes(
      x = percentage_increase_in_population * 100,
      y = change_in_odds,
      colour = age_band
    )
  ) +
  labs(
    ...
    y = 'Change in Odds [%]',
    x = 'Percentage increase in age band population [%]'
  )
