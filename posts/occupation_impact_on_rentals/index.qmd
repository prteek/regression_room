...
abstract:
    ...
    authority and neighbourhood information to capture location-specific patterns.
    We fit binomial models that account for age mix and hierarchical geography,
    using a balanced sample across local authorities and MSOAs and excluding London.
    Higher‑skilled groups rent less, professionals are ~55% less likely
    to rent than elementary occupations; rentership is highest among 25–34 year olds.
    Including local authority and MSOA random effects improves model fit and reveals
    spatial heterogeneity, with town centres renting more than suburbs. Limitations
    include age‑by‑occupation counts for all workers (not HRPs), cross‑sectional data,
    and non‑London focus. Results describe associations, indicating rental demand is
    shaped by occupational structure, age composition, and local context.
...
Primarily, these seem to areas in the city centers where rentership is much higher than what the model predicts and can be seen on the map too. <br/>
...
ordered_residuals_glm_base <- dataset %>%
  ...
  mutate(
    actual_renting_proportion = round(renting_total / occupation_total, 2),
    predicted_proportion = round(predict(glmod_base, ., type = "response"), 2)
  ) %>%
...
msoa_geometries <- athena_sql(glue::glue_sql(
  ...
  msoa_list = ordered_residuals_glm_base %>% head(10) %>% pull(msoa),
  .con = acon
))
...
### Introducing location context in the model
 ...
 This is an opportunity for us to introduce location information (LAD and MSOA) in the model. We shall be mindful of the fact that the MSOA are randomly chosen and that MSOA are nested inside LAD.
...
Diagnostics have improved and residuals are smaller than before, so the addition of MSOAs in model is worthwhile. <br/>
...
The model captures renting behaviour across different occupations reasonably well and does not seem to have biased results for any. <br/>
An avenue for improvement can be with more information relevant to *Elementary occupations* and *Professional occupations* where the model seems to make large errors compared to others.

```{r}
df_plot %>%
  ggplot() +
  geom_boxplot(aes(x = occupation, y = .resid)) +
...
df_plot %>%
  ggplot(aes(x = log(occupation_total), y = .resid)) +
...
```{r}
ordered_residuals_glm_base %>%
  rename(predicted_proportion_base = predicted_proportion) %>%
  select(
    msoa,
    lad_name,
    occupation,
  ...
  mutate(
    predicted_proportion_glmer = round(
      predict(glmermod, ., type = "response"),
      2
    )
  ) %>%
...
df_sf <- msoa_geometries %>%
  inner_join(
    dataset_full %>%
      group_by(msoa) %>%
      summarise(residual = mean(abs(.resid))),
    by = "msoa"
  ) %>%
  mutate(
    geometry = st_as_sfc(msoa_polygon_wkt, crs = 4326) # WKT → sfc
...
ggplot() +
  annotation_map_tile(
    type = "https://cartodb-basemaps-a.global.ssl.fastly.net/light_all/${z}/${x}/${y}.png",
    zoomin = 0,
    progress = "none"
  ) +
  geom_sf(
  ...
  scale_fill_gradientn(
    colors = RColorBrewer::brewer.pal(9, "RdYlBu")[9:1], # reverse RdYlBu
    name = "MAE (prob)"
  )
...
MSOA nested inside LAD are slighly more dispersed and have longer tails, but it doesn't seem too problematic to affect reliability of the model.
...
In general any Occupation category is less likely to rent compared to *Elementary Occupations*, in particular *Professionals* are nearly 55% less likely (0.44x) to rent. <br/>
There is intuitive sense to the effects as higher paid occupations are less likely to rent, except interestingly that unlike observations in raw data, *Managers* are not the least likely renters when age is taken into account.
...
Age plays a key role in likelihood of renting too. Rentership is highly sensitive to population in 25-34 year age bracket. With a 10% increase in population in that age bracket, odds increase by approx `{r} glmermod %>% fixef() %>% tidy() %>% filter(str_detect(names, 'age_25_34')) %>% pull(x) %>% prod(log(1.1)/10) %>% exp() %>% sum(-1) %>% prod(100) %>% round(1)`% <br/>
...
Supposing that a real estate investment firm wants to introduce new rental supply or wants to sell its existing portfolio in a location where the target consumer is employed as a *Professional* or in *Sales and service*, the elasticities associated with these changes can be estimated (as an example +/- 50 units change is demonstrated below). <br/>
This exercise is helpful with a test for the fact that new supply does not significantly change the underlying renting propensity. This would mean that under existing conditions, new supply can likely be absorbed by the market. <br/>
It is important to note that this evaluation is only from supply point of view. It is possible that price elasticity of supply can limit absorption much earlier than what is proposed here. Neverthless, the idea here tests for stability in the market and that may implicitly imply stability of some phenomenon that affect price elasticity.
...
sample <- dataset_full %>%
  ...
  mutate(
    ...
    actual = renting_total / occupation_total,
    se_lo = predicted -
      1.96 * sqrt((1 - predicted) * predicted / occupation_total),
    se_hi = predicted +
      1.96 * sqrt((1 - predicted) * predicted / occupation_total)
  )
...
sample %>%
  cross_join(tibble(
    delta_change_in_population = seq(-50, 50, by = 2)
  )) %>%
  # pivot_longer(starts_with('age')) %>%
  mutate(age_25_34 = pmax(age_25_34 + delta_change_in_population, 0)) %>%
  # pivot_wider(names_from = 'name', values_from = 'value') %>%
  mutate(
    .pred = predict(glmermod, ., type = "response", allow.new.levels = TRUE)
  ) %>%
  ggplot() +
  geom_line(aes(
    x = age_25_34 / 2, # to convert population to households
    y = .pred,
    colour = msoa,
  )) +
  geom_point(
    ...
    data = sample %>%
      pivot_longer(c(predicted, actual)) %>% # Removing actuals from the plot because error bars are not standard errors of p-estimate but rather the sampling variability introduced by probabilistic process
  ...
  facet_wrap(~occupation, scales = "free_x", ncol = 3) +
  labs(
    subtitle = "Elasticity of age (random MSOA)",
    x = "HRP in age bracket 25-34 years",
    y = "Propensity to rent"
  )
