...
The deviance residuals are not constrained to have mean zero so the mean level of the plot is not of interest. There is approximately even variation as the linear prediction varies (except perhaps at the very top and bottom ends). We do not consider this as a major issue at this point.
...
Among the categorical variables only *property_type* appears to have some degree of association with residuals. We will not address this at this point as the model is still likely to change.
...
Residuals exhibit a strong patter against *log(active days)*. This is suggestive of a more flexible treatment of active days in the model (we shall opt for splines later).
...
train <- train %>% mutate(predout=ifelse(predprob < 0.65, "no", "yes"), is_reduced=ifelse(is_5percent_reduced < 0.5, "no", "yes"))

xtabs(~ is_reduced + predout, train)

```

The selection of threshold is going to be biased to favour Sensitivity since there may be desire to be precise about revisions. For now we just balance the 2 attributes and observe that upon choosing a reasonable threshold of *p=0.65* the classification table already looks much better.


## Conclusion
There is much scope for improvement still. The problem of predicting price reduction is tricky, there are a lot of subjective factors at play with each property along with market related factors. <br/>
